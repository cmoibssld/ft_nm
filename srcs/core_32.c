...
Elf32_Shdr *elf_sheader_32(Elf32_Ehdr *hdr, off_t size, bool swap) {
	...
	if (e_shoff == 0 || e_shoff > (uint32_t)size || 
        (uint32_t)(e_shentsize * e_shnum) > (uint32_t)size - e_shoff) {
    return NULL;
}
...
void print_symbols_32(t_nm_sym *sort_array, int output_size, bool swap, t_nm_args *nm_args) {
	...
	for (int i = 0; i < output_size; i++) {
        ...
        if (st_shndx == SHN_UNDEF) {
            ...
            printf("%08x ", (unsigned int)st_value);
        }
...
int get_symbols_32(Elf32_Ehdr *hdr, Elf32_Shdr *symtab, Elf32_Shdr *sections, char *strtab, char *shstrtab, bool swap, off_t size, t_spec *spec, uint32_t strtab_size, t_nm_args *nm_args) {
	...
	for (int i = 1; i < sym_count; i++) {
        ...
        if (should_keep) {
            ...
            sort_array[output_size].c = get_nm_char_32(symbols[i], sections, shstrtab, e_shnum, swap);
            output_size++;
        }
    }
	
    print_symbols_32(sort_array, output_size, swap, nm_args);
    free(sort_array);
    return(0);
}
...
int core_engine_32(char *addr, off_t size, t_spec *spec, t_nm_args *nm_args, bool swap) {
	...
	if (get_symbols_32(hdr, symtab, sections, strtab, shstrtab, swap, size, spec, str_size, nm_args)) {
