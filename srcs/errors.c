...
int  print_error_message(const char *problematic_arg)
{
  ...
  write(STDERR_FILENO, buff, expected_bytes);
  free(buff);
  return (0);
}

int  print_personnal_error(const char * restrict arg, const char *msg)
{
  ssize_t  expected_bytes;
  char    *buff;

  expected_bytes = ft_strlen("ft_nm: : \n") + ft_strlen(arg) + ft_strlen(msg);
  buff = (char *)ft_calloc(expected_bytes + 1, sizeof(char));
  if (buff == NULL)
    return (errno);
  sprintf(buff, "ft_nm: %s: %s\n", arg, msg);
